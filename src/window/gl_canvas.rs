...
use gl;
use glutin::{
    self, dpi::LogicalSize, ContextBuilder,
    GlRequest, WindowedContext,
};
...
impl AbstractCanvas for GLCanvas {
    fn open(
        ...
        let window = unsafe {
            WindowedContext::new_shared(window, context, &events).unwrap()
        };
        ...
        self.events.poll_events(|event| match event {
            glutin::event::Event::WindowEvent  { event, .. } => match event {
                ...
                glutin::event::WindowEvent::MouseWheel {
                    ...
                    let (x, y) = match delta {
                        glutin::event::MouseScrollDelta::LineDelta(dx, dy) => (dx as f64, dy as f64),
...
fn translate_key(button: Option< glutin::event::VirtualKeyCode>) -> Key {
    if let Some(button) = button {
        match button {
             ...
             glutin::event::VirtualKeyCode::Add => Key::Add,
             ...
             glutin::event::VirtualKeyCode::Decimal => Key::Decimal,
             glutin::event::VirtualKeyCode::Divide => Key::Divide,
             ...
             glutin::event::VirtualKeyCode::Multiply => Key::Multiply,
             ...
             glutin::event::VirtualKeyCode::Subtract => Key::Subtract,
